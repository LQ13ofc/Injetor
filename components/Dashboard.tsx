...
const Dashboard: React.FC<DashboardProps> = ({ stats, setStats, addLog, onOpenHub, settings, setSettings }) => {
  ...
  const handleInject = async () => {
    ...
    if (!stats.isAdmin) {
        addLog("CRITICAL: Running without Admin Privileges. Injection likely to fail.", "WARN", "KERNEL");
    }
    ...
    if (settings.ghostMode) addLog("OpSec: Ghost Mode & Time Stomping (Real) Active", 'INFO', 'STEALTH');
    ...
    try {
      ...
      if (result.success) {
        ...
        if (cleanError.includes("Arquitetura")) {
            addLog("TIP: Target is 32-bit (WoW64). Use a 32-bit Injector build.", 'WARN', 'SYSTEM');
        }
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 gap-4">
        {/* Target Selector */}
        <div className="bg-[#141417] border border-white/5 p-4 rounded-xl relative group hover:border-white/10 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {/* Method Selector */}
             <div className="bg-[#141417] border border-white/5 p-4 rounded-xl hover:border-white/10 transition-colors">
      ...
      {/* Main Action Area */}
      <div className={`bg-[#121215] border rounded-2xl p-8 flex flex-col items-center text-center space-y-6 transition-all duration-500 relative overflow-hidden ${
        ...
        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all relative z-10 ${
          ...
          {stats.processStatus === 'ERROR' ? <ShieldAlert size={32} /> : stats.processStatus === 'INJECTED' ? <Ghost size={32} /> : <Zap size={32} fill="currentColor" />}
        </div>
